...
const KelasTable = ({ dataKelas, currentPage ,totalBarisYangDibutuhkan}) => {
    const tinggiBaris = 60; // px
    const jumlahBarisKosong = Math.max(0, totalBarisYangDibutuhkan - dataKelas.length);
        ...
        <div className="w-100 h-100 flex-grow-1 overflow-auto">
            <div className="mt-2">
                ...
                <div className="d-flex fw-bold border-bottom align-items-center w-100">
                    <div className="text-center" style={{ width: "5%" }}>No</div>
                    <div style={{ width: "30%" }}>Nama Kelas</div>
                    <div style={{ width: "30%" }}>Kategori</div>
                    <div style={{ width: "15%" }}>Harga</div>
                    <div className="text-center" style={{ width: "10%" }}>Status</div>
                ...
                {dataKelas.map((kelas, index) => (
                    ...
                    >
                        ...
                        {/* Nama Kelas */}
                        <div style={{ width: "30%" }}>{kelas.nama_kelas}</div>
                        ...
                        <div className="d-flex flex-wrap" style={{ width: "30%" }}>
                            {kelas.kategori.map((kat, i) => (
                                <span
                                    key={i}
                                    className={`badge ${colorsKategory[kat.id % colorsKategory.length]} me-1 mb-1`}
                                    style={{ fontSize: "12px" }}
                                >
                                    {kat.nama}
                                </span>
                        ...
                        {/* Harga */}
                        <div style={{ width: "15%" }}>{formatPrice(kelas.harga)}</div>
                        ...
                        <div className="d-flex justify-content-center" style={{ width: "10%" }}>
                            <button
                                ...
                                onClick={() => navigate(`/admin/master/kelas/detail/${kelas.kelas_id}`)}
                            >
                                Detail
                            </button>
                        </div>
...
export const KelasList = () => {
    ...
    const serviceGetAllKelas = useCallback(async (page = 1, search = "") => {
        try {
            setIsLoading(true)
            const response = await axios.get(`${config.APIURL}/kelas/list?page=${page}&limit=${totalBarisYangDibutuhkan}&nama_kelas=${search}`, {
                headers: {
        ...
        >
            ...
            <div className="bg-white shadow-lg d-flex flex-column" style={{ width: "95%", height: "95%", borderRadius: "10px", paddingTop: "10px", paddingBottom: "10px" }}>
                ...
                <div className="flex-grow-1 d-flex justify-content-center align-items-center">
                    {!isLoading ?
                        (
                            <KelasTable dataKelas={dataKelas} currentPage={currentPage} totalBarisYangDibutuhkan={totalBarisYangDibutuhkan}/>
                        ) :
                        (
